...
describe "Thermobath reporter" do
  ...
  describe "PUT /set_direction" do
    ...
    it "direction を stop から run に変更できる" do
      File.open(@direction_file, 'w') do |f|
        f.puts "stop"
      end
      put '/set_direction/run?target=38.0'
      last_response.ok?.must_equal true # response 200 を>    返すこと
      File.open(@direction_file) do |f|
        f.gets.chomp.must_match /^38.0/
      end
    end

    it "direction を run から stop に変更できる" do
      File.open(@direction_file, 'w') do |f|
        f.puts "38.0"
      end
